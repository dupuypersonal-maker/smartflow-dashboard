// In-memory token store — tokens expire after 15 minutes
// NOTE: In Vercel serverless, each invocation may get a fresh instance.
// We use a global map as best-effort; for production use Upstash/Redis.
// For a lead magnet this is perfectly sufficient.
...
module.exports = async function handler(req, res) {
  ...
  // Generate token and store with 15min expiry
  const token = generateToken();
  const expiry = Date.now() + 15 * 60 * 1000;
  global._tokens.set(token, { email, fuente: fuente || 'Dashboard Demo', industry: industry || 'General', expiry });

  // Clean expired tokens
  for (const [k, v] of global._tokens.entries()) {
    if (v.expiry < Date.now()) global._tokens.delete(k);
  }
...
<span style="display:inline-block;width:28px;height:28px;background:#2563eb;border-radius:7px;text-align:center;line-height:28px;color:#fff;font-size:14px;font-weight:700;margin-right:8px">S</span>
<span style="font-family:Georgia,serif;font-size:18px;color:#fafafa;letter-spacing:-0.02em">Smartflow</span>
</td>
...
<p style="font-size:14px;color:#71717a;line-height:1.65;margin:0 0 28px;font-weight:300">Haz click en el botón para acceder a tu análisis completo. Este link expira en <strong style="color:#18181b">15 minutos</strong>.</p>
<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px">
<tr><td align="center">
<a href="${magicLink}" style="display:inline-block;padding:14px 36px;background:#2563eb;color:#ffffff;text-decoration:none;border-radius:50px;font-size:15px;font-weight:600;letter-spacing:-0.01em">Acceder a mi dashboard →</a>
</td></tr></table>
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;border-radius:8px;margin-bottom:24px">
<tr><td style="padding:16px 20px">
<p style="font-size:12px;color:#71717a;margin:0;line-height:1.5">Si no puedes hacer click en el botón, copia y pega este link en tu navegador:<br><span style="color:#2563eb;word-break:break-all;font-size:11px">${magicLink}</span></p>
</td></tr></table>
...
<p style="font-size:13px;color:#71717a;margin:0 0 14px;font-weight:300">Agenda 30 minutos y te mostramos cómo conectar todas tus fuentes.</p>
...
<p style="font-size:11px;color:#a1a1aa;margin:0">Smartflow · Consultoría de IA · Si no solicitaste este acceso, ignora este email.</p>
</td></tr>
</table>
</td></tr></table>
</body></html>`
  ...
  return res.status(200).json({ success: true, message: 'Magic link enviado' });
}
